    ...
    return supabase
      ...
      .on(
        ...
        (payload: RealtimePostgresChangesPayload<any>) => {
          // Convert database record to application model using type mapper
          const mappedRequest = mapDbRedemptionToRedemptionRequest(payload.new);
          
          // Create a fully compliant RedemptionRequest object
          const redemptionRequest: RedemptionRequest = {
            id: mappedRequest.id,
            requestDate: typeof mappedRequest.requestDate === 'object' 
              ? toISOString(mappedRequest.requestDate) 
              : mappedRequest.requestDate,
            tokenAmount: mappedRequest.tokenAmount,
            tokenType: mappedRequest.tokenType || "",
            redemptionType: mappedRequest.redemptionType || "standard",
            status: this.mapStatusToApprovalStatus(mappedRequest.status),
            sourceWalletAddress: mappedRequest.sourceWalletAddress || "",
            destinationWalletAddress: mappedRequest.destinationWalletAddress || "",
            conversionRate: mappedRequest.conversionRate || 0,
            investorName: mappedRequest.investorName || "",
            investorId: mappedRequest.investorId || "",
            isBulkRedemption: mappedRequest.isBulkRedemption || false,
            investorCount: mappedRequest.investorCount || 1,
            approvers: mappedRequest.approvers || [],
            requiredApprovals: mappedRequest.requiredApprovals || 0
          };
