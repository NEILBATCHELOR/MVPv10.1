...
const MultiSigWalletDashboard: React.FC<MultiSigWalletDashboardProps> = ({
  ...
  const [newTxBlockchain, setNewTxBlockchain] = useState<string>("ethereum");

  // Update blockchain when wallet changes
  useEffect(() => {
    if (wallet) {
      // @ts-ignore - blockchain property might not be in the type but exists at runtime
      const walletBlockchain = wallet.blockchain || "ethereum";
      setNewTxBlockchain(walletBlockchain);
    }
  }, [wallet]);
    ...
    const initializeWallet = async () => {
      try {
        ...
        if (walletId === "demo-wallet") {
          ...
          // Create a mock wallet with the correct type structure
          const mockWallet: Wallet & { blockchain?: string } = {
            id: "demo-wallet",
          ...
          // Create mock pending transactions with the correct structure
          ...
          // Create mock executed transactions with the correct structure
