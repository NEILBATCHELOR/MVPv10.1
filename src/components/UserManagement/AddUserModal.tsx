...
const AddUserModal = ({
  ...
  const updateFormData = (updates: Partial<typeof formData>) => {
    updateState(setFormData, updates);
  };
  ...
  const handleAutoGenerateChange = (checked: boolean) => {
    updateFormData({ 
      ...
      password: checked ? generateRandomPassword() : formData.password 
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !formData.role ||
      !keyPair ||
      (!formData.autoGeneratePassword && !formData.password)
    ) {
      toast({
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `User ${formData.name} has been created successfully${formData.sendInvitation ? " and an invitation email has been sent" : ""}`,
      });

      // Call the onSubmit callback with the user data
      onSubmit({ name: formData.name, email: formData.email, role: formData.role });
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex">
                  <Input
                    ...
                    type="text"
                    placeholder={
                      formData.autoGeneratePassword ? "Auto-generated" : "Enter password"
                    }
              ...
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => updateFormData({ sendInvitation: checked === true })}
                />
                <Label htmlFor="send-invitation">Send email invitation</Label>
              </div>
              {formData.sendInvitation && (
                <p className="text-xs text-gray-500">
                  An email will be sent to {formData.email || "the user"} with login
