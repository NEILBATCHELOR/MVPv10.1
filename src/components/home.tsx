...
const Home: React.FC<HomeProps> = () => {
  ...
  const handleManageSubscription = async (projectId: string) => {
    try {
      ...
      setSelectedSubscriptionProject(transformedProject as any);
  ...
  const handleSubscribe = async (planId: string) => {
    if (selectedSubscriptionProject) {
      try {
        ...
        // Create a proper subscription object with explicit typing
  ...
  const handleCancelSubscription = async () => {
    if (
      ...
      try {
        ...
        // Mock cancelSubscription function since it doesn't exist
        const cancelSubscription = async (id: string, reason: string) => {
          console.log(`Cancelling subscription ${id} with reason: ${reason}`);
          return { success: true };
        };
  ...
  const handleRenewSubscription = async () => {
    if (
      ...
      try {
        ...
        // Mock renewSubscription function since it doesn't exist
        const renewSubscription = async (id: string, planId: string) => {
          console.log(`Renewing subscription ${id} with plan: ${planId}`);
          return { success: true };
        };
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 container mx-auto py-6 px-4 md:px-6">
        ...
        {isLoading ? (
          ...
          <SubscriptionManager 
            currentSubscription={{ projectId: selectedSubscriptionProject.id }}
            onSubscribe={(planId) => {
