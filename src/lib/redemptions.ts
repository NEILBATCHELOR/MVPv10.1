...
export async function createRedemptionRequest(
  ...
  const { item, error: mappingError } = handleSupabaseSingleResponse(
    data,
    error,
    mapDbRedemptionToRedemptionRequest,
  );

  if (mappingError) {
    console.error("Error creating redemption request:", mappingError);
    return null;
  }

  return item as RedemptionRequest | null;
}
...
export async function getRedemptionRequests(): Promise<RedemptionRequest[]> {
  ...
  if (mappingError) {
    console.error("Error fetching redemption requests:", mappingError);
    throw mappingError;
  }

  return items as RedemptionRequest[];
}
...
export async function getRedemptionRequest(
  ...
  if (mappingError) {
    console.error(`Error fetching redemption request ${id}:`, mappingError);
    return null;
  }
...
export const updateRedemptionRequest = async (
  ...
  try {
    ...
    return {
      ...
      redemptionType: data.redemption_type || "standard",
      status: data.status,
      ...
      isBulkRedemption: data.is_bulk_redemption || false,
      investorCount: data.investor_count || 1,
      approvers: data.approvers || [],
      requiredApprovals: data.required_approvals || 0,
    };
  } catch (error) {
