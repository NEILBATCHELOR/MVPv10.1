  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        ...
        created_at,
        updated_at,
        investors(name, email, wallet_address, type, kyc_status)
      `,
      )
    ...
    return (
      data?.map((subscription) => ({
        ...subscription,
        fiat_amount: subscription.fiat_amount, // Use fiat_amount directly
        subscription_id: subscription.id, // Use id as subscription_id if not present
        investor: {
          name: subscription.investors?.name || "Unknown",
          email: subscription.investors?.email || "",
          wallet_address: subscription.investors?.wallet_address,
          type: subscription.investors?.type || "individual",
          kyc_status: subscription.investors?.kyc_status || "not_started",
        },
      })) || []
    );
  } catch (error) {
...
export const getSubscriptionStats = async (projectId: string) => {
  try {
    ...
    // Ensure data is treated as an array of the correct type
    ...
    const confirmedCount = subscriptions.filter((sub) => sub.confirmed).length || 0;
    const allocatedCount = subscriptions.filter((sub) => sub.allocated).length || 0;
    const distributedCount = subscriptions.filter((sub) => sub.distributed).length || 0;
    const totalAmount =
      subscriptions.reduce((sum, sub) => sum + (sub.fiat_amount || 0), 0) || 0;
...
export const getInvoice = async (
  ...
  try {
    ...
    if (error) throw error;
    // Use a more explicit type conversion with 'unknown' as intermediate step
